...
public class GradientVolume {
    ...
    private void compute() {
        ...
        //set gradients of all pixels as zero
        for (int i=0; i<data.length; i++) {
            data[i] = zero;
        }
        //calculate x,y and z gradient for each voxel and set gradient
        for (int x=1; x<dimX-1; x++) {
            for (int y=1; y<dimY-1; y++) {
                for (int z=1; z<dimZ-1; z++){
                    float xGrad = (volume.getVoxel(x+1, y, z) - volume.getVoxel(x-1, y, z))/2.0f;
                    float yGrad = (volume.getVoxel(x, y+1, z) - volume.getVoxel(x, y-1, z))/2.0f;
                    float zGrad = (volume.getVoxel(x, y, z+1) - volume.getVoxel(x, y, z-1))/2.0f;
                    VoxelGradient gradient = new VoxelGradient(xGrad, yGrad, zGrad);
	...
	private void interpolate(VoxelGradient g0, VoxelGradient g1, float factor, VoxelGradient result) {
            //interpolate for values of x,y and z coordinates 
